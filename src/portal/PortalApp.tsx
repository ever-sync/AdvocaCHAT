...
import { type PortalContext, portalMembershipFingerprint } from "./types";
        ...
        : (
          ...
          >
            <p role="status" className="break-words text-sm">
              ...
              {sentTo}. Confira a caixa de entrada e o spam. A solicitação não
              confirma a entrega.
            </p>
...
function PortalActivation({ entry }: { entry: PortalEntry }) {
  ...
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const diligence = entry.path.startsWith("/portal/diligencias");
  const home = diligence ? "/portal/diligencias" : "/portal";
  if (!entry.activation || entry.invalidActivation) {
    return (
      <div className="mx-auto max-w-lg space-y-4">
        ...
        <Button asChild>
          <Link to={home}>Entrar no portal</Link>
        </Button>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-lg space-y-5">
      <div>
        <h1 className="text-xl font-semibold">
          {diligence ? "Aceite sua diligência" : "Aceite seu atendimento"}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          ...
          <section className="space-y-4 rounded-xl border bg-card p-5">
            ...
            >
              {busy
                ? "Confirmando…"
                : diligence
                ? "Aceitar acesso desta diligência"
                : "Aceitar acesso deste atendimento"}
            </Button>
          </section>
        )}
    </div>
...
function PortalLayout({ entry }: { entry: PortalEntry }) {
  ...
  const diligence = location.pathname.startsWith("/portal/diligencias");
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b bg-card">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3 px-4 py-4">
          <Link
            to={diligence ? "/portal/diligencias" : "/portal"}
            className="flex min-w-0 items-center gap-2 font-semibold"
          >
            <Scale className="h-6 w-6 shrink-0 text-primary" />
            <span>
              {diligence ? "Portal de diligências" : "Portal do cliente"}
            </span>
          </Link>
          {auth.session && (
            <Button
        ...
        {auth.session && (
          <nav
            ...
            className="mx-auto flex max-w-5xl flex-wrap gap-4 px-4 pb-3 text-sm"
          >
            <Link className="underline" to="/portal">Meus atendimentos</Link>
            <Link className="underline" to="/portal/diligencias">
              Minhas diligências
            </Link>
          </nav>
        )}
      ...
      <main className="mx-auto max-w-5xl px-4 py-6 sm:py-10">
        <Routes>
          ...
          />
          <Route
            path="/portal/diligencias"
            element={<DiligenceWorkspace login={<PortalLogin />} />}
          />
          <Route
