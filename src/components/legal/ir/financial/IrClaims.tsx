...
export function IrClaims(props: FinancialPanelProps) {
  ...
  const decidedClaims = new Set((events.data ?? []).filter((event) => ["decision_granted", "decision_partial", "decision_denied"].includes(event.event_type)).map((event) => event.claim_id));
