...
import { deriveIrReadiness, downloadIrCaseDossier, type IrDossierData } from "@/lib/legal-ir-dossier";
import { OperationPanel } from "../operations/OperationPanel";
import { legalErrorMessage } from "../legal-ui";
import { irKey, type IrPanelProps } from "./ir-ui";
...
export function IrCaseOverview(props: IrPanelProps) {
  ...
  const [payers, incomes, evidence, reviews, checklist, assessments] = useQueries({ queries: [
  ...
  const data: IrDossierData = { context: props.ir, payers: payers.data ?? [], incomes: incomes.data ?? [], evidence: evidence.data ?? [], reviews: reviews.data ?? [], checklist: checklist.data ?? [], assessments: assessments.data ?? [] };
  const readiness = deriveIrReadiness(data);
  const loading = [payers, incomes, evidence, reviews, checklist, assessments].some((query) => query.isFetching);
  const failed = [payers, incomes, evidence, reviews, checklist, assessments].some((query) => query.error);

  async function exportDossier() {
    setDownloading(true);
    try {
      const before = await getIrCaseContext(props.legalCase.id);
      const [freshPayers, freshIncomes, freshEvidence, freshReviews, freshChecklist, freshAssessments] = await Promise.all([
      ...
      const result = { context: after, payers: freshPayers, incomes: freshIncomes, evidence: freshEvidence, reviews: freshReviews, checklist: freshChecklist, assessments: freshAssessments } satisfies IrDossierData;
