...
import { CASE_STATUSES, CASE_TYPES, legalDate, selectClassName } from "@/components/legal/legal-ui";
...
export default function LegalCases() {
  ...
  const cases = useQuery({
    queryKey: ["legal", workspace?.user_id, workspace?.tenant_id, "cases", deferredSearch, status, customerId],
    queryFn: () => listLegalCases({ search: deferredSearch, status, customer_id: customerId }),
    ...
    {context.isPending ? <LegalLoading /> : context.error ? <LegalError error={context.error} retry={() => void context.refetch()} /> : workspace ? <>
      {workspace.enabled && id ? detail.isPending ? <LegalLoading /> : detail.error ? <LegalError error={detail.error} retry={() => void detail.refetch()} /> : detail.data ? <LegalCaseDetail key={detail.data.id} legalCase={detail.data} workspace={workspace} /> : <LegalEmpty title="Caso indisponível">O caso não foi encontrado ou seu acesso não está autorizado.<div className="mt-4"><Button asChild variant="outline"><Link to="/casos">Voltar aos casos</Link></Button></div></LegalEmpty> : <>
        ...
        {!workspace.enabled || showSettings ? <>
          ...
          {cases.isPending ? <LegalLoading /> : cases.error ? <LegalError error={cases.error} retry={() => void cases.refetch()} /> : cases.data?.length ? <div className="space-y-3"><p className="text-sm text-muted-foreground">{cases.data.length} {cases.data.length === 1 ? "caso neste filtro" : "casos neste filtro"}{cases.isFetching ? " · Atualizando…" : ""}</p><div className="grid gap-3 lg:grid-cols-2">{cases.data.map((legalCase) => <Card key={legalCase.id} className="transition-colors hover:border-primary/40"><CardContent className="space-y-4 p-5"><div className="flex items-start justify-between gap-3"><Link className="min-w-0 flex-1 break-words font-semibold hover:text-primary hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" to={`/casos/${legalCase.id}`}>{legalCase.title}</Link><ArrowUpRight className="h-4 w-4 shrink-0 text-muted-foreground" aria-hidden /></div><div className="flex flex-wrap gap-2"><Badge variant={legalCase.status === "ativo" ? "default" : "secondary"}>{CASE_STATUSES[legalCase.status]}</Badge><Badge variant="outline">{CASE_TYPES[legalCase.case_type]}</Badge>{legalCase.area ? <span className="text-sm text-muted-foreground">{legalCase.area}</span> : null}</div><div className="rounded-lg bg-muted/40 p-3"><p className="text-xs font-medium text-muted-foreground">Próxima providência</p><p className="mt-1 break-words text-sm">{legalCase.next_action || "Definir próxima providência"}</p>{legalCase.next_action_due_at ? <p className="mt-2 flex items-center gap-1.5 text-xs text-muted-foreground"><CalendarClock className="h-3.5 w-3.5" aria-hidden />{legalDate(legalCase.next_action_due_at, true)}</p> : null}</div>{legalCase.status === "aguardando" && legalCase.wait_reason ? <p className="text-sm text-muted-foreground">Aguardando: {legalCase.wait_reason}</p> : null}</CardContent></Card>)}</div>{cases.data.length === 200 ? <p className="text-sm text-muted-foreground">Exibindo os 200 casos atualizados mais recentemente. Use a busca e os filtros para localizar os demais.</p> : null}</div> : <LegalEmpty title={search || status !== "all" ? "Nenhum caso encontrado" : "Seu primeiro caso começa aqui"}>{search || status !== "all" ? "Ajuste a busca ou a situação selecionada." : "Os casos aos quais você receber acesso aparecerão nesta área."}{workspace.can_create ? <div className="mt-4"><Button onClick={() => setNewCaseOpen(true)}><Plus className="mr-2 h-4 w-4" aria-hidden />Criar caso</Button></div> : null}</LegalEmpty>}
        </>}
      </>}
