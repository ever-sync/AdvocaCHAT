...
export const LEGAL_DOCUMENT_MIME_TYPES = ["application/pdf", "image/png", "image/jpeg", "text/plain", "text/csv"] as const;

/** Validate declared formats; text is stored verbatim and every download is an attachment. */
export function validateLegalDocument(bytes: Uint8Array, mimeType: string): string | null {
  ...
  return "Formato inválido. Envie PDF, PNG, JPG, TXT ou CSV em UTF-8.";
}
