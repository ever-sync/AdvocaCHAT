 ...
 * telefone ("+55 (21) 99999-9999"), "Cliente", "Contato WhatsApp", "Sem nome" ou
 * qualquer coisa não-significativa (telefone cru, JID, vazio). Quando chega o nome
 * REAL do lead (inbound ou perfil do WhatsApp), esses devem ser substituídos.
 */
function isAutoGeneratedDisplayName(value: string | null | undefined) {
  const normalized = String(value ?? "").trim();
  if (!normalized) return true;
  // Rótulo de telefone ("+55 (21) 99999-9999") não tem letras → não é significativo.
  ...
  // Legado: "WhatsApp (21) 99999-9999" tinha letras (passava como significativo),
  // então precisa de detecção explícita até o backfill limpar os antigos.
...
export async function ensureChat(
  ...
  const payload = {
    ...
    // rótulo de telefone ("+55 (11) 99999-9999"), não em "Sem nome". O placeholder
    // não é "significativo", então é substituído assim que um nome real aparecer.
    display_name: pickBestDisplayName(
      customer?.nome,
      params.displayName,
      existing?.display_name,
    ) ?? formatAutoCustomerName(normalized),
...
/**
 * Links chat to CRM negotiation when the flow explicitly needs it.
 *
 * Antes esta rotina criava a negociação automaticamente ao entrar mensagem
 * no WhatsApp. Agora ela fica desativada por padrão para não empurrar todo
 * lead para a primeira etapa do CRM sem ação manual.
 */
export async function ensureLeadFromChat(admin: AdminClient, chatId: string) {
  console.info("ensureLeadFromChat desativado: criação automática de negociação via WhatsApp foi removida.", {
    chatId,
  });
  return null;
}
