-- F5 case finance integration regression. Run ONLY in a disposable/local or isolated staging
...
reset role;
select pg_temp.assert_true(not has_schema_privilege('legal_portal','public','USAGE'),'external identity cannot call inherited public financial RPCs');
rollback;
\echo 'F5 case finance regression PASS (rolled back)'
