...
select pg_temp.expect_error($q$select public.ir_record_document_review(pg_temp.lid('doc_medical'),'{}','pending','Needs type confirmation','{"confirmed_document_type":"unknown"}')$q$,'22023','medical document review requires a confirmed document type');
update legal_test_ids set id=(public.ir_record_document_review(pg_temp.lid('doc_medical'),'{"issuer":"present","signature":"unclear","date":"present"}','pending','Professional assessment needed','{"issuer_name":"Synthetic physician","professional_registration":"SYNTHETIC","document_nature":"private","confirmed_document_type":"medical_report","issued_on":"2020-01-02","reported_onset_on":"2019-01-01"}')).id where name='review';
