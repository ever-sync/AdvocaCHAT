-- F4 integration regression. Run ONLY in a disposable/local or isolated staging
-- database after migrations, with postgres privileges. Synthetic data rolls back.
-- psql -v ON_ERROR_STOP=1 -f supabase/tests/ir_calculations_and_requests.sql
...
-- CONCURRENCY_FIXTURE_READY

-- A refused scenario keeps its reasons but exposes no partial or misleading total.
create function pg_temp.assert_incomplete(p_payload jsonb,p_code text) returns void language plpgsql as $$
declare r public.ir_calculation_versions;begin
 r:=public.ir_create_calculation_version(pg_temp.lid('case'),p_payload);
 perform pg_temp.assert_true(r.status='incomplete' and exists(select 1 from jsonb_array_elements(r.refusals) x where x->>'code'=p_code) and r.result->>'baseline_tax' is null and r.result->>'proposed_tax' is null and r.result->>'hypothesis_difference' is null and r.result->>'withheld_reported' is null and r.result->'groups'='[]'::jsonb,'incomplete scope refuses total: '||p_code);
end;$$;
select pg_temp.assert_incomplete(jsonb_set(pg_temp.calc_payload(),'{tax_residency}','"unknown"'),'residency_not_covered');
select pg_temp.assert_incomplete(jsonb_set(pg_temp.calc_payload(),'{tax_residency}','"non_resident"'),'residency_not_covered');
select pg_temp.assert_incomplete(jsonb_set(pg_temp.calc_payload(),'{inventory_complete}','false'),'inventory_incomplete');
select pg_temp.assert_incomplete(jsonb_set(pg_temp.calc_payload(),'{parameter_ids}','[]'),'parameter_coverage_missing');
select pg_temp.assert_incomplete(pg_temp.calc_payload() #- '{adjustments,0,period_review_id}','period_not_reviewed');
select pg_temp.expect_error($q$select public.ir_create_calculation_version(pg_temp.lid('case'),jsonb_set(pg_temp.calc_payload(),'{assessment_id}',to_jsonb(gen_random_uuid())))$q$,'42501','assessment cannot cross case or be invented');
select pg_temp.expect_error($q$select public.ir_create_calculation_version(pg_temp.lid('case'),jsonb_set(pg_temp.calc_payload(),'{adjustments,0,proposed_taxable}','0'))$q$,'22023','calculation adjustment requires monetary string');

update legal_test_ids set id=(public.ir_create_tax_import(pg_temp.lid('case'),pg_temp.lid('csv'),'Incomplete original row','csv','[{"raw_data":{"gross":"unreadable"}}]')).id where name='bad_import';
select pg_temp.assert_true((select gross is null and taxable is null and withheld is null and legal_deductions is null from public.ir_tax_entries where import_id=pg_temp.lid('bad_import')),'unknown monetary inputs stay null rather than silently becoming zero');
select pg_temp.expect_error($q$select public.ir_review_tax_import(pg_temp.lid('bad_import'),'reviewed','Cannot infer missing values')$q$,'22023','missing original facts block approval');
select pg_temp.assert_incomplete(pg_temp.calc_payload(),'scope_entries_missing');
select public.ir_update_tax_entry((select id from public.ir_tax_entries where import_id=pg_temp.lid('bad_import')),(pg_temp.tax_lines()->0)||'{"gross":"90071992547409.01","taxable":"90071992547409.01","source_line":"LARGE"}'::jsonb);
select pg_temp.assert_true((select gross='90071992547409.01' and to_jsonb(e)->>'gross'='90071992547409.01' and jsonb_typeof(to_jsonb(e)->'gross')='string' from public.ir_tax_entries e where import_id=pg_temp.lid('bad_import')),'large canonical decimal persists and serializes as exact text');
select public.ir_review_tax_import(pg_temp.lid('bad_import'),'reviewed','All explicit large synthetic amounts checked');
select pg_temp.assert_incomplete(jsonb_set(jsonb_set(jsonb_set(pg_temp.calc_payload(),'{periodicity}','"annual"'),'{month}','null'),'{import_ids}',jsonb_build_array(pg_temp.lid('import'),pg_temp.lid('bad_import'))),'minimum_tax_not_covered');
select public.ir_review_tax_import(pg_temp.lid('bad_import'),'rejected','Large boundary fixture is intentionally not part of the client inventory');
select pg_temp.assert_true((select count(*) from public.ir_tax_import_reviews where import_id=pg_temp.lid('bad_import'))=2 and (select gross='90071992547409.01' from public.ir_tax_entries where import_id=pg_temp.lid('bad_import')),'reviewed import rejection preserves original values and both decisions');
select pg_temp.expect_error($q$delete from public.ir_tax_import_reviews$q$,'42501','import review history is append only');

update legal_test_ids set id=(public.ir_create_tax_import(pg_temp.lid('case'),pg_temp.lid('csv_duplicate'),'Duplicate uploaded bytes','csv',pg_temp.tax_lines())).id where name='duplicate_import';
select public.ir_review_tax_import(pg_temp.lid('duplicate_import'),'reviewed','Both duplicate rows individually checked');
select pg_temp.assert_incomplete(jsonb_set(pg_temp.calc_payload(),'{import_ids}',jsonb_build_array(pg_temp.lid('import'),pg_temp.lid('duplicate_import'))),'duplicate_document_rows');
select public.ir_review_tax_import(pg_temp.lid('duplicate_import'),'rejected','Duplicate evidence overlap identified');
update legal_test_ids set id=(public.ir_create_tax_import(pg_temp.lid('case'),pg_temp.lid('csv_duplicate'),'Partial overlapping extract','csv',jsonb_build_array(pg_temp.tax_lines()->0,(pg_temp.tax_lines()->1)||'{"source_line":"DIFFERENT"}'::jsonb))).id where name='duplicate_import';
select public.ir_review_tax_import(pg_temp.lid('duplicate_import'),'reviewed','Partially overlapping rows checked');
select pg_temp.assert_incomplete(jsonb_set(pg_temp.calc_payload(),'{import_ids}',jsonb_build_array(pg_temp.lid('import'),pg_temp.lid('duplicate_import'))),'duplicate_document_rows');
select public.ir_review_tax_import(pg_temp.lid('duplicate_import'),'rejected','Even one overlapping proof line blocks a combined total');

do $$declare kind text;r public.ir_tax_imports;begin
 foreach kind in array array['thirteenth','rra','regressive','foreign','other'] loop
  r:=public.ir_create_tax_import(pg_temp.lid('case'),pg_temp.lid('csv'),'Unsupported regime fixture','manual',jsonb_build_array((pg_temp.tax_lines()->0)||jsonb_build_object('income_tax_kind',kind,'source_line','EXTRA-'||kind)));
  perform public.ir_review_tax_import(r.id,'reviewed','Classified explicitly without assuming ordinary taxation');
  perform pg_temp.assert_incomplete(jsonb_set(pg_temp.calc_payload(),'{import_ids}',jsonb_build_array(pg_temp.lid('import'),r.id)),'income_not_covered');
  perform public.ir_review_tax_import(r.id,'rejected','Isolated unsupported-regime fixture excluded');
 end loop;
end;$$;
update legal_test_ids set id=(public.ir_create_tax_parameter_version(jsonb_build_object('parameter_key','annual_2026','title','Annual ordinary synthetic table','periodicity','annual','calendar_year',2026,'exercise',2027,'valid_from','2026-01-01','valid_until','2026-12-31','body',pg_temp.tax_body('a26')))).id where name='annual_parameter';
select public.ir_record_parameter_validation(pg_temp.lid('annual_parameter'),'{"taxable":"72000.00","legal_deductions":"0.00","deduction_mode":"most_favorable","expected_tax":"3387.01","expected_source":"Independent annual C10","review_note":"Ordinary annual scope explicitly reviewed"}');
select public.ir_review_tax_parameter_version(pg_temp.lid('annual_parameter'),'approved','Synthetic annual coverage and arithmetic review');
do $$declare r public.ir_calculation_versions;begin
 r:=public.ir_create_calculation_version(pg_temp.lid('case'),pg_temp.calc_payload()||jsonb_build_object('periodicity','annual','month',null,'parameter_ids',jsonb_build_array(pg_temp.lid('annual_parameter'))));
 perform pg_temp.assert_true(r.status='draft' and r.result->>'baseline_tax'='0.00' and r.result->>'proposed_tax'='0.00' and jsonb_array_length(r.result->'groups')=1 and r.result->'groups'->0->>'payer_id' is null,'annual scope aggregates covered sources once and uses an annual table');
end;$$;
select pg_temp.assert_true(public.ir_read_calculation_report(pg_temp.lid('calc2'))->>'is_current'='false','later declarations, payments and imports stale prior memory but retain historical export');

-- New proof affects the F3 basis; this must not suppress a verifiable past fact.
update legal_test_ids set id=(public.legal_prepare_document(pg_temp.lid('case'),'fiscal','Later receipt','later.pdf','application/pdf',10)).id where name='fiscal';
reset role;
insert into storage.objects(bucket_id,name,metadata) select 'legal-case-documents',storage_path,'{"size":10}'::jsonb from public.legal_case_documents where id=pg_temp.lid('fiscal');
select pg_temp.login('owner','service_role');set local role service_role;
select public.legal_finalize_document(pg_temp.lid('fiscal'),repeat('7',64));
reset role;set local role authenticated;select pg_temp.login('owner');
select pg_temp.assert_incomplete(pg_temp.calc_payload(),'assessment_not_current');
select pg_temp.expect_error($q$select public.ir_review_claim_strategy(pg_temp.lid('claim2'),'Recheck','Recheck','Old assessment cannot authorize new acts')$q$,'22023','stale legal basis blocks new strategy approval');
select public.ir_record_claim_event(pg_temp.lid('claim2'),jsonb_build_object('event_type','protocol','description','Past external protocol documented after basis changed','occurred_on',(current_date-3)::text,'document_id',pg_temp.lid('fiscal'),'protocol_reference','LATE-EVIDENCED-PROTOCOL'));
select public.ir_record_recovery(pg_temp.lid('allocation2'),jsonb_build_object('amount','1.00','received_on',(current_date-1)::text,'channel','judicial_payment','document_id',pg_temp.lid('fiscal'),'reference','LATE-RECEIPT','idempotency_key',gen_random_uuid()));
select pg_temp.assert_true(public.ir_get_financial_context(pg_temp.lid('case'))->'principal_balances'->0->>'received'='21.00','evidenced external cash can be registered despite a stale analysis');
select pg_temp.assert_true(not exists(select 1 from public.legal_case_events where description like '%protected clinical%' or metadata::text like '%Synthetic protected%' or metadata::text like '%382.88%'),'shared audit contains identifiers and generic events, not medical rationale or monetary memory');

-- Actual tenant, membership, category, edit/review and feature boundaries.
select pg_temp.login('finance');
select pg_temp.assert_true((select count(*) from public.ir_tax_entries)>0 and (select count(*) from public.ir_calculation_versions)=0 and (select count(*) from public.ir_claims)=0 and (select count(*) from public.ir_period_reviews)=0,'fiscal-only member sees fiscal records but no combined legal-medical memory');
select pg_temp.assert_true(public.ir_get_financial_context(pg_temp.lid('case'))->>'can_fiscal'='true' and public.ir_get_financial_context(pg_temp.lid('case'))->>'can_calculate'='false' and public.ir_get_financial_context(pg_temp.lid('case'))->'overlaps'='[]'::jsonb,'fiscal context gives neutral balances without confidential claim links');
select pg_temp.expect_error($q$select public.ir_read_calculation_report(pg_temp.lid('calc'))$q$,'42501','fiscal-only cannot export medical snapshot');
select pg_temp.expect_error($q$select public.ir_create_calculation_version(pg_temp.lid('case'),pg_temp.calc_payload())$q$,'42501','fiscal-only cannot create combined calculation');
select pg_temp.expect_error($q$select public.ir_review_tax_import(pg_temp.lid('import'),'rejected','Not owner')$q$,'42501','fiscal editor cannot approve or reject reviewed source data');
select pg_temp.expect_error($q$select public.ir_verify_payment_principal(pg_temp.lid('principal'),'Not owner')$q$,'42501','fiscal editor cannot verify principal');
select pg_temp.login('owner');select public.legal_set_case_member(pg_temp.lid('case'),pg_temp.lid('member'),false,true,true);
select pg_temp.login('member');
select pg_temp.assert_true(public.ir_read_calculation_report(pg_temp.lid('calc'))->>'is_current'='false','combined read-only member may export historical authorized memory');
select pg_temp.expect_error($q$select public.ir_create_calculation_version(pg_temp.lid('case'),pg_temp.calc_payload())$q$,'42501','combined read-only member cannot mutate calculations');
select pg_temp.expect_error($q$select public._ir4_compute(pg_temp.lid('case'),pg_temp.calc_payload())$q$,'42501','internal deterministic helper cannot bypass workflow ACL');
select pg_temp.expect_error($q$select public._ir4_principal_balance(pg_temp.lid('principal'))$q$,'42501','internal balance helper cannot expose arbitrary case');
select pg_temp.login('owner');select public.legal_set_case_member(pg_temp.lid('case'),pg_temp.lid('member'),true,true,false);
select pg_temp.login('member');
select pg_temp.assert_true((select count(*) from public.ir_tax_entries)=0 and (select count(*) from public.ir_calculation_versions)=0,'medical-only category cannot see fiscal or combined rows');
select pg_temp.expect_error($q$select public.ir_read_calculation_report(pg_temp.lid('calc'))$q$,'42501','revoked fiscal grant immediately blocks historical export');
select pg_temp.login('same_admin');
select pg_temp.assert_true((select count(*) from public.ir_tax_entries)=0 and (select count(*) from public.ir_claims)=0,'same-tenant administrator has no implicit case access');
select pg_temp.expect_error($q$select public.ir_get_financial_context(pg_temp.lid('case'))$q$,'42501','unassigned admin cannot read financial context');
select pg_temp.login('outsider');
select pg_temp.assert_true((select count(*) from public.ir_tax_parameter_versions)=0 and (select count(*) from public.ir_tax_entries)=0,'platform-selected tenant does not override physical tenant');
select pg_temp.expect_error($q$select public.ir_read_calculation_report(pg_temp.lid('calc'))$q$,'42501','cross-tenant platform administrator cannot export protected memory');
select pg_temp.login('owner');select public.legal_set_workspace_enabled(false);
select pg_temp.assert_true((select count(*) from public.ir_tax_entries)=0 and (select count(*) from public.ir_tax_parameter_versions)=0,'disabled workspace hides fiscal data and parameters');
select pg_temp.expect_error($q$select public.ir_read_calculation_report(pg_temp.lid('calc'))$q$,'42501','feature disabled blocks report even for owner');
select public.legal_set_workspace_enabled(true);
reset role;select pg_temp.login('owner','service_role');update public.profiles set status='inactive' where id=pg_temp.lid('owner');set local role authenticated;select pg_temp.login('owner');
select pg_temp.assert_true((select count(*) from public.ir_tax_entries)=0,'inactive owner loses fiscal visibility');
select pg_temp.expect_error($q$select public.ir_read_calculation_report(pg_temp.lid('calc'))$q$,'42501','inactive owner cannot export a previously accessible report');
reset role;
select 'F4 integration regression PASS — all synthetic records rolled back' as result;
rollback;
